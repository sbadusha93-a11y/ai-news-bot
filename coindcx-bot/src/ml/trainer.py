...
class MLTrainer:
    ...
    async def train(
        ...
        try:
            ...
            return await loop.run_in_executor(None, self._train_sync, dfs, symbols)
    ...
    def _train_sync(
        ...
        if "target" not in combined.columns:
            logger.warning("No target column in combined features")
            return {"status": "error", "error": "no_target_column"}
        ...
        if not isinstance(y, pd.Series):
            try:
                ...
                logger.error(f"Cannot convert target to Series: {e}")
                return {"status": "error", "error": f"invalid_target_type: {type(y).__name__}"}
